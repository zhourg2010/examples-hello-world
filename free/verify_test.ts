...
Deno.test("没有 PUSH_KEY 或者对不上 → 401", async () => {
  // 钥匙用 ASCII:HTTP header 的值必须是 Latin-1,中文会让 new Request 直接抛
  assertEq(await statusOf({ results: [] }, "wrong-key"), 401, "钥匙不对必须 401");
});

// 下面这些都带对的钥匙。没配 PUSH_KEY 的环境里一律 401,那样测不出东西,跳过。
const keyed = KEY ? Deno.test : Deno.test.ignore;

keyed("GET 之类的方法 → 405", async () => {
  ...
  const res = await handleFreeVerify(req);
  await res.body?.cancel();
  assertEq(res.status, 405, "只收 POST");
});

keyed("请求体不是 JSON → 400", async () => {
  assertEq(await statusOf("{这不是json"), 400, "解析不了要明确报 400,不能 500");
});

keyed("results 不是数组 / 空数组 → 400", async () => {
...
keyed("uriHash 形状不对 → 400,而不是等外键去挡", async () => {
...
keyed("latencyMs 不合法 → 400,但可以不填", async () => {
